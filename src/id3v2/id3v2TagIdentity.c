...
bool id3v2WriteTagSizeRestriction(Id3v2TagHeader *header, const uint8_t bits) {
    ...
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 6,
                                                  readBit(bits, 0));
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 7,
                                                  readBit(bits, 1));
 ...
 * @details This function will fail if bits is greater than 0x03 as the standard only defines 3
 * unique options: 0b00, 0b01, 0b10, and 0b11. This function returns 0 if it fails and 1
 * otherwise.
 * @param header
 * @param bits
 * @return true
...
bool id3v2WriteTextFieldsSizeRestriction(Id3v2TagHeader *header, const uint8_t bits) {
    ...
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 4,
                                                  readBit(bits, 1));
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 3,
                                                  readBit(bits, 0));
...
/**
 * @brief Sets the image size restriction to the value of bits within an extended tag headers
 * restrictions.
 * @details This function will fail if bits is greater than 0x03 as the standard only defines 3
 * unique options: 0b00, 0b01, 0b10, and 0b11. This function returns 0 if it fails and 1
 * @param header
...
bool id3v2WriteImageSizeRestriction(Id3v2TagHeader *header, const uint8_t bits) {
    ...
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 1,
                                                  readBit(bits, 1));
    header->extendedHeader->restrictions = setBit(header->extendedHeader->restrictions, 0,
                                                  readBit(bits, 0));
 ...
 * @details If this function fails it will return -1 otherwise it will return the value of the tag
 * size restriction.
 * @param header
 * @return int
 */
 ...
 * @details If this function fails it will return -1 otherwise, it will return the value of the
 * indicator.
 * @param header
 * @return int
 */
...
/**
 * @brief Reads the text field size restriction from the restriction byte held by the extended
 * header.
 * @details If this function fails it will return -1 otherwise, it will return the value of the
 * indicator.
 * @param header
...
/**
 * @brief Reads the Image Encoding restriction from the restriction byte held by the extended
 * header.
 * @details If this function fails it will return -1 otherwise, it will return the value of the
 * indicator.
 * @param header
...
/**
 * @brief Converts an extended tag header into a buffer. if this function fails it will
 * return NULL otherwise, an uint8_t pointer.
 * @details This functions output format is dependent on the version of the tag header. Some fields
 * are not supported in all versions and will be omitted even if present within the structure. The
 * returned buffer is heap allocated if not NULL and must be freed by the user.
 * @param ext
 * @param version
 * @param outLength
 * @return uint8_t*
...
uint8_t *id3v2ExtendedTagHeaderSerialize(Id3v2ExtendedTagHeader *ext, const uint8_t version,
    ...
    case ID3V2_TAG_VERSION_4: {
        ...
        if (ext->crc) {
            ...
            toWrite = sizeof(size_t) - offset > 5 ? 5 : ((int) sizeof(size_t)) - offset;
 ...
 * @details Some fields are not supported in all versions and will be omitted even if present within
 * the structure. The returned string is heap allocated and must be freed by the user.
 * @param ext
 * @param version
 * @return char*
 */
 ...
 * return NULL otherwise, an uint8_t pointer.
 * @details Some fields are not supported in all versions and will be omitted even if present within
 * the supplied structure. The returned buffer is heap allocated if not NULL and must be freed by
 * the user.
 * @param header
 * @param uintSize
 * @param outLength
...
uint8_t *id3v2TagHeaderSerialize(Id3v2TagHeader *header, const uint32_t uintSize,
    ...
    // versions are all covered above but just in case a user manually changes the version
    default:
        byteStreamDestroy(stream);
        *outLength = 0;
        return NULL;
    ...
    if (id3v2ReadExtendedHeaderIndicator(header)) {
        size_t extSize = 0;
        uint8_t *ext = id3v2ExtendedTagHeaderSerialize(header->extendedHeader, header->majorVersion,
                                                       &extSize);
...
/**
 * @brief Converts a tag header into its structures representation as JSON.
 * @details Some fields are not supported in all versions and will be omitted even if present within
 * the structure. The returned string is heap allocated and must be freed by the user.
 * @param header
 * @return char *
